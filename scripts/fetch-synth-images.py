...
def main():
    ...
    # Checked before any download: finding out after the images are rewritten
    # leaves new assets beside stale attribution.
    if not os.path.isdir(os.path.dirname(DATA_FILE)):
        raise SystemExit(f'No directory for {os.path.relpath(DATA_FILE, ROOT)} — '
                         'the data file has moved; re-point DATA_FILE first.')
