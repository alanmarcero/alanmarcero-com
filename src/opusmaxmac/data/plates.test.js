...
const PLATE_DIR = path.join(__dirname, '..', '..', '..', 'public', 'opus-max-mac', 'plates');
...
describe('srcSetFor / sourceFor', () => {
  it('describes each candidate with the width it really is', () => {
    ...
    expect(srcSetFor(plate)).toBe(
      '/opus-max-mac/plates/prophet-08-320.webp 320w, '
      + '/opus-max-mac/plates/prophet-08-640.webp 640w',
    );
  });
  ...
  it('offers a single candidate when only one derivative exists', () => {
    ...
    expect(srcSetFor(plate)).toBe('/opus-max-mac/plates/jp-08-281.webp 281w');
  });

  it('falls back to the widest derivative', () => {
    entries.forEach(([, plate]) => {
      expect(sourceFor(plate)).toBe(`/opus-max-mac/plates/${plate.slug}-${plate.width}.webp`);
    });
  });
});
